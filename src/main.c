...
static void uart_task()
{
    ...
    while (1)
    {
        ...
        if (len > 0)
        {
            but1_send = 0x40;
            but3_send = 0x40;
            ESP_LOGI("uart", "pressed LR\n");
        }
        else
        {
            but1_send = 0;
            but3_send = 0;
            //ESP_LOGI("uart", "released LR\n");
        }
    }
    // uart_flush(UART_NUM);
    //  while (1) {
...
esp_err_t set_bt_address()
{
    ...
    {
        //前3桁はNintendo OUI Rangeの物にする
        //具体的にはD4:F0:57:XX:XX:XXになる
        bt_addr[0] = 0xD4;
        bt_addr[1] = 0xF0;
        bt_addr[2] = 0x57;
        for (int i = 3; i < 8; i++)
